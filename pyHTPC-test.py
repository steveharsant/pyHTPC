...
# Get time
sys_time = jsonify  # Can this be modified to import python code from another file dynamically?


plugin_dirs = os.listdir('./plugins/') # This needs error handling to ensure only directories are used.



def parse_plugins(directory):
        config = '['
        count = 0

        for path in directory glob.glob(os.path.join(directory, '*.json'))
